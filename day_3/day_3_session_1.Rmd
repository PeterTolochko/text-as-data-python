...
```{r}
library(quanteda.textmodels)
model.svm <- textmodel_svm(dfm_train, dfm_train$polarity)
...
setwd("/Users/fabiennelind/ucloud/Lehre/Automated Text Analysis/PhD course text analysis/2024/data")
write.csv(reviews_test, "rotten_tomatoes_nb_svm_test.csv", row.names = F)

reviews_train <- subset(reviews, id %in% id_train)
reviews_train$polarity_nb <- NA
reviews_train$polarity_svm <- NA
write.csv(reviews_train, "rotten_tomatoes_nb_svm_train.csv", row.names = F)

reviews_all <- rbind(reviews_test, reviews_train)
write.csv(reviews_all, "rotten_tomatoes_nb_svm.csv", row.names = F)
...
filename: The desired filename to save the plot
...
```{r}
library(ggplot2)
plot_conf_mat(reviews_test, "blue", "confusion_matrix.png")
